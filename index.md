...
To easily initialize a new repository with content, first create a new repository without `README`, `.gitignore`, or `license`, and then run `npx wicg init "My Awesome API"`.  Enter the requested text and select "bikeshed" for the Spec preprocessor.  This command will then create those files and the spec source file, `index.bs`.

It is convenient to set up a "GitHub Pages site" and actions for automatically publishing and serving the html file that will be generated by Bikeshed after each change of your source file. To do that, make the following two changes.
