...
Create an initial "empty" specification file as your source, e.g. `spec.bs` or `index.bs`.
 ...
 You can copy this [Minimal template](http://go/gh/WICG/starter-kit/blob/main/templates/index.bs), or you can run `bikeshed template > spec.bs` to generate the following minimal template.
...
You can run Bikeshed locally on your spec with `bikeshed spec spec.bs` to generate `spec.html`.
...
- Submit updates to your specification source, e.g. `spec.bs`, to GitHub, which should then trigger the action that updates `spec.html`.
